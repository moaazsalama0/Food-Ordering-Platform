...
export default function UploadMealImages() {
  ...
  const upload = async (mealId) => {
    try {
      ...
      // either PUT to meals/:id/ or use separate upload endpoint
      await api.put(`${endpoints.MEALS}${mealId}/`, fd, { headers: { ...authHeaders(), "Content-Type": "multipart/form-data" }});
      alert("Uploaded");
    } catch (err) {
      console.error(err);
      alert("Upload failed");
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {meals.map(m => (
          <div key={m.id} className="bg-white p-4 rounded shadow flex items-center gap-4">
            <img src={m.image || m.image_url} alt={m.name} className="w-20 h-20 object-cover rounded" />
